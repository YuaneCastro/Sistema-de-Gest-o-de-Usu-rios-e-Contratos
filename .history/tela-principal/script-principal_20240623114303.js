document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadContratos() {
        try {
            // Obter os contratos do banco de dados (substitua 1 pelo ID do usuário correto)
            const contratos = await listContratosByUserId(1);
    
            // Selecionar a lista de contratos onde os contratos serão exibidos
            const contratosList = document.getElementById('contratos-list');
    
            // Limpar a lista atual de contratos (caso haja algum)
            contratosList.innerHTML = '';
    
            // Iterar sobre cada contrato retornado e criar um elemento para exibição
            contratos.forEach((contrato) => {
                ...
                // Adicionar evento de clique para mostrar detalhes do contrato (opcional)
                contratoItem.addEventListener('click', () => {
                    alert(`Contrato: ${contrato.titulo}`);
                    // Aqui você pode implementar a lógica para mostrar detalhes do contrato
                });
    
                // Adicionar o item de contrato à lista de contratos
